      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <div className="flex items-center justify-between mb-6 sm:mb-8 gap-3">
          <div className="min-w-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-white truncate">My Library</h1>
          ...
          <button
            ...
            disabled={uploading}
            className="px-4 sm:px-5 py-2.5 sm:py-3 bg-accent hover:bg-accent/90 text-white font-semibold rounded-xl transition flex items-center gap-2 disabled:opacity-50 text-sm shrink-0"
          >
            {uploading ? (
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {books.map((book) => (
              ...
              >
                <div className="h-32 sm:h-40 bg-gradient-to-br from-panel to-surface flex items-center justify-center">
                  <BookOpen className="w-12 h-12 sm:w-16 sm:h-16 text-accent/30 group-hover:text-accent/60 transition" />
                </div>
            ...
            <div
              onClick={() => fileInputRef.current?.click()}
              className="bg-surface/20 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center p-6 sm:p-10 hover:border-accent/30 transition cursor-pointer min-h-[200px] sm:min-h-[280px]"
            >
              <Plus className="w-8 h-8 sm:w-10 sm:h-10 text-gray-500 mb-3" />
              <p className="text-gray-500 text-sm">Add a new book</p>
            </div>
          </div>
        )}
      </main>
